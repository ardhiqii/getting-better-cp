...
The first line of input contains $N$ ($1 \leq N \leq 100$).

Each of the remaining $N$ lines contains a single statement and is of the form `D S`, where:

- **D** is a character describing a direction:
  - `N` = north
  - `E` = east
  - `S` = south
  - `W` = west
- **S** is the number of steps taken in that direction ($1 \leq S \leq 10$).

### Output Format

**File:** `mowing.out`

Please determine the maximum possible value of $x$ such that FJ never steps on a cell with cut grass. If FJ never visits any cell more than once, please output $-1$.
